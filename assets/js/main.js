...
async function loadScarcityBadge() {
  try {
    ...
    if (data.custom_badge_text) {
      ...
      if (sct)
        sct.textContent = data.custom_sticky_text || data.custom_badge_text;
      return;
    }
...
async function loadFaq() {
  ...
  const FALLBACK_FAQ = [
    {
      question: "혼자 가도 괜찮을까요?",
      answer:
        "네, 전혀 문제 없습니다! ODD PARTY는 처음 만나는 분들이 어색함 없이 어울릴 수 있도록 다양한 아이스브레이킹 프로그램을 운영합니다. 혼자 오시는 분들이 오히려 더 다양한 만남을 경험하세요.",
    },
    {
      question: "참가 연령 제한이 있나요?",
      answer:
        "만 20세 이상 37세 이하를 대상으로 합니다. 보다 편안한 분위기를 위해 연령대를 적절히 조율하여 운영하고 있습니다.",
    },
    {
      question: "입금 후 취소/환불이 가능한가요?",
      answer:
        "파티 4일 전까지 전액 환불이 가능합니다. 3일 전~당일 취소는 환불이 불가합니다. 취소 문의는 카카오톡 채널로 연락해 주세요.",
    },
    {
      question: "어떤 프로그램이 진행되나요?",
      answer:
        "아이스브레이킹 게임, 테이블 토크, 자유 네트워킹 타임 등으로 구성되며, 매회 조금씩 새로운 프로그램이 추가됩니다. 음료와 간단한 스낵도 제공됩니다.",
    },
    {
      question: "신청 후 어떻게 확인하나요?",
      answer:
        "입금 완료 후 24시간 이내 문자 또는 카카오톡으로 확정 안내를 보내드립니다. 입금 계좌는 신청 완료 화면에서 확인하실 수 있습니다.",
    },
  ];
  ...
  try {
    ...
    Object.entries(content).forEach(([key, val]) => {
      if (
        !val ||
        key === "pricing" ||
        key === "scarcity_override" ||
        key === "scarcity-badge-text" ||
        key === "sticky-cta-text" ||
        key === "instagram-id"
      )
        return;
      ...
      try {
        const pricing =
          typeof content.pricing === "string"
            ? JSON.parse(content.pricing)
            : content.pricing;
        const grid = document.getElementById("main-price-grid");
        if (grid) {
          const branches = Object.keys(pricing).filter(
            (k) => k !== "part2_base" && k !== "part2_discount",
          );
          if (branches.length > 0) {
            ...
            branches.forEach((branch) => {
              ...
              card.innerHTML =
                ...
                '<span class="price-card-badge">' +
                esc(branch) +
                "</span>" +
                '<p class="price-card-name">' +
                esc(branch) +
                "점</p>" +
                ...
                '<span class="price-val price-val-accent">' +
                esc(fmtPrice(p.female)) +
                "</span>" +
                '<span style="font-size:var(--fs-xs);color:var(--muted)">원</span></div>' +
              ...
              card.addEventListener(
                ...
                (function (b) {
                  return function () {
                    location.href = "form.html?branch=" + encodeURIComponent(b);
                  };
                })(branch),
              );
...
/* sharePage() is now defined in common.js */
